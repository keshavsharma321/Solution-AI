...
const services = {
  ...
  vciso: {
    ...
    description:
      "AI-powered Virtual Chief Information Security Officer providing 24/7 cybersecurity strategy, compliance, and risk management.",
    features: [
  ...
  "enterprise-ai": {
    ...
    description:
      "Secure AI assistant for executives and growing teams, offering scalable, enterprise-grade automation and insights.",
    features: [
...
// Helper function to split array into chunks and join each chunk into a paragraph
const createParagraphs = (items: string[], chunksCount = 3) => {
  if (items.length <= chunksCount) {
    return [items.join(" ")]
  }

  const result: string[] = []
  const itemsPerChunk = Math.ceil(items.length / chunksCount)

  for (let i = 0; i < chunksCount; i++) {
    const start = i * itemsPerChunk
    const end = Math.min(start + itemsPerChunk, items.length)
    if (start < items.length) {
      result.push(items.slice(start, end).join(" "))
    }
  }
...
const ServiceDetailPage: React.FC = () => {
  ...
  const featureParagraphs = createParagraphs(service.features, 3)
  const benefitParagraphs = createParagraphs(service.benefits, 2)
            ...
            <div className="">
              <div className="container mx-auto px-4">
                <div className="w-24 h-24 rounded-full mx-auto mb-6 bg-white/10 flex items-center justify-center relative">
                  <div className="bg-white/80 rounded-full p-2">{service.icon}</div>
                </div>
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <AnimatedSection>
            <div className="bg-white rounded-xl shadow-lg p-8 mb-12">
              <h2 className="text-3xl font-bold mb-8 text-violet-900 border-b border-violet-100 pb-4">Key Features</h2>
              <div className="space-y-6">
                {featureParagraphs.map((paragraph, index) => (
                  <p key={index} className="text-gray-700 leading-relaxed text-lg font-normal tracking-wide">
                    {paragraph}
                  </p>
                ))}
              </div>
          ...
          <AnimatedSection>
            <div className="bg-white rounded-xl shadow-lg p-8">
              <h2 className="text-3xl font-bold mb-8 text-violet-900 border-b border-violet-100 pb-4">Benefits</h2>
              <div className="space-y-6">
                {benefitParagraphs.map((paragraph, index) => (
                  <p key={index} className="text-gray-700 leading-relaxed text-lg font-normal tracking-wide">
                    {paragraph}
                  </p>
                ))}
          ...
          <AnimatedSection className="mt-16 text-center">
            <div className="bg-gradient-to-r from-violet-600 to-indigo-600 rounded-2xl p-8 md:p-12">
              ...
              <Button
                ...
                className="bg-violet-500 text-white hover:bg-violet-50"
              >
                Contact Us
              </Button>
            </div>
